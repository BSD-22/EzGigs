                    ...
                    {sortedChats.map(([chatId, chat]) => {
                        ...
                        return (
                            <Link
                                ...
                                className="block bg-white rounded-xl md:rounded-2xl overflow-hidden border border-[#D3D9C9] p-3 md:p-4 hover:shadow-lg transition-all duration-300"
                            >
                                <div className="flex items-center gap-3 md:gap-4">
                                    <div className="w-10 h-10 md:w-12 md:h-12 rounded-full bg-[#4A5043]/10 flex items-center justify-center text-lg md:text-xl">
                                        {otherParticipant?.name?.[0]?.toUpperCase() || "?"}
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <h3 className="font-bold text-[#2C3228] text-sm md:text-base truncate">
                                            {otherParticipant?.name || "Unknown User"}
                                        </h3>
                                        {chat.lastMessage ? (
                                            <p className="text-xs md:text-sm text-[#4A5043] truncate">
                                                {chat.lastMessage.text}
                                            </p>
                                        ) : (
                                            <p className="text-xs md:text-sm text-[#4A5043]/70 italic">
                                                No messages yet
                                            </p>
                                        )}
                                    </div>
