      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 md:mb-8">
        <div>
          <Header
            ...
            description="Find and sell tickets for your favorite events 🎫"
          />
        </div>
        <form
          action="/home/marketplace/sell"
              ...
              <Link
                ...
                <div className="relative h-[120px] sm:h-[140px] md:h-[160px]">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-2 md:p-3">
                    <h3 className="text-sm md:text-lg font-bold text-white line-clamp-1 group-hover:text-white/90 transition-colors">{listing.ticket.name}</h3>
                ...
                <div className="p-2 md:p-4 flex flex-col gap-2 md:gap-3">
                  ...
                  <div className="flex flex-col w-full">
                    <div className="flex flex-wrap items-center gap-1 md:gap-1.5 mb-1.5 md:mb-2">
                      <span className="px-1.5 md:px-2 py-0.5 bg-[#4A5043]/5 rounded-full text-[10px] md:text-xs font-medium text-[#4A5043]">{listing.categoryName}</span>
                      <span className="px-1.5 md:px-2 py-0.5 bg-emerald-50 rounded-full text-[10px] md:text-xs font-medium text-emerald-600">Seat {listing.seatNumber}</span>
                    </div>
                  ...
                  <div className="flex flex-col gap-1.5">
                    <div className="flex items-center gap-1.5">
                      <div className="w-4 h-4 md:w-6 md:h-6 rounded-full bg-[#4A5043] flex items-center justify-center">
                        <span className="text-[9px] md:text-xs font-medium text-white">{listing.user.name[0].toUpperCase()}</span>
                      </div>
                      <span className="text-[10px] md:text-xs text-[#4A5043] truncate max-w-[80px] md:max-w-[120px]">{listing.user.name}</span>
                    </div>

                    {/* Price */}
                    <div className="flex flex-col">
                      <div className="text-xs md:text-lg font-bold text-[#2C3228]">Rp {listing.price.toLocaleString("id-ID")}</div>
                      {listing.price !== listing.ticket.seatCategories[0].price && (
                        <div className="flex items-center gap-0.5 md:gap-1">
                          <span className="text-[8px] md:text-xs line-through text-[#4A5043]/70">Rp {listing.ticket.seatCategories[0].price.toLocaleString("id-ID")}</span>
                          {listing.price < listing.ticket.seatCategories[0].price ? (
