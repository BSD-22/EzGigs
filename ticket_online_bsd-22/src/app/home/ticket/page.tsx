...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#F4F6F0] via-white to-[#E8EDE1]">
              ...
              <div className="relative h-[280px] w-full overflow-hidden bg-[#2C3228]">
                ...
                <div className="absolute top-4 right-4">
                  <div className="bg-white/95 backdrop-blur-sm rounded-2xl p-2 text-center min-w-[80px]">
                    <p className="text-[#2C3228] text-2xl font-bold">{new Date(ticket.date).getDate()}</p>
                    <p className="text-[#4A5043] text-sm font-medium">{new Date(ticket.date).toLocaleDateString("id-ID", { month: "short" })}</p>
                  </div>
                </div>

                {/* Event Title on Image */}
                <div className="absolute bottom-0 left-0 right-0 p-6">
                  <h3 className="text-2xl font-bold text-white mb-2 group-hover:text-[#D3D9C9] transition-colors">{ticket.name}</h3>
                  ...
                  {ticket.seatCategories.map((category) => (
                    <button
                      ...
                      className={`w-full p-4 rounded-2xl text-left transition-all duration-300 ${
                        category.availableSeats > 0 ? "bg-[#F4F6F0] hover:bg-[#E8EDE1] border border-[#D3D9C9]" : "bg-gray-100 cursor-not-allowed opacity-60"
                      }`}>
                      <div className="flex justify-between items-start">
                        <div className="flex-1 space-y-1.5">
                          <div className="flex items-center gap-2">
                            <p className="font-semibold text-[#2C3228]">{category.name}</p>
                            <span className="px-2 py-0.5 text-xs font-medium bg-[#4A5043]/10 text-[#4A5043] rounded-full">{category.availableSeats} left</span>
                          </div>
                          {userSubscription !== "free" && getDiscountBadge()}
                          <div className="flex items-baseline gap-2">
                            {userSubscription !== "free" && <p className="text-sm text-gray-400 line-through">Rp {category.price.toLocaleString("id-ID")}</p>}
                            <p className="text-base font-medium text-[#4A5043]">Rp {Math.round(calculateDiscountedPrice(category.price)).toLocaleString("id-ID")}</p>
                          </div>
                        </div>
                        <div className="h-10 w-10 rounded-xl bg-[#4A5043]/10 flex items-center justify-center flex-shrink-0 ml-4">
      ...
      {/* Purchase Modal - Light Theme with Scroll */}
      <Dialog
        open={isModalOpen}
