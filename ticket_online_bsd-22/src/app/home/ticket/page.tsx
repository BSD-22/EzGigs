...
export default function Home() {
  ...
  const getDiscountBadge = () => {
    if (userSubscription === "premium") {
      return <span className="px-2 py-0.5 text-xs font-medium bg-green-500/10 text-green-600 rounded-full">5% Silver Discount</span>;
    }
    if (userSubscription === "vip") {
      return <span className="px-2 py-0.5 text-xs font-medium bg-purple-500/10 text-purple-600 rounded-full">8% Gold Discount</span>;
    }
    return null;
  };
      ...
      <div className="flex-1 p-7 overflow-auto">
        <div className="flex flex-col gap-6 mb-10">
          <div className="flex justify-between items-center">
            <div>
              <h1 className="text-5xl font-black text-[#2C3228]">Upcoming Events</h1>
              <p className="text-[#4A5043] mt-2">Discover and book your next unforgettable experience 🎉</p>
            </div>
            <div className="flex items-center gap-3">
              <button 
                onClick={() => setViewMode("grid")}
                className={`p-2 rounded-lg ${viewMode === "grid" ? "bg-[#4A5043] text-white" : "bg-gray-100"}`}
              >
                📱
              </button>
              <button 
                onClick={() => setViewMode("list")}
                className={`p-2 rounded-lg ${viewMode === "list" ? "bg-[#4A5043] text-white" : "bg-gray-100"}`}
              >
                📝
              </button>
            </div>
          ...
          <div className="flex gap-4 flex-wrap items-center">
            <div className="flex-1 min-w-[280px]">
              <input
                ...
                className="w-full px-4 py-3 rounded-xl bg-white border border-[#D3D9C9]"
              />
            </div>
            ...
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="px-4 py-3 rounded-xl bg-white border border-[#D3D9C9]"
                />
              ...
              <button
                ...
                className="px-4 py-3 rounded-xl bg-white border border-[#D3D9C9] flex items-center gap-2 hover:bg-[#F4F6F0]"
              >
                <span>Date</span>
                {sortOrder === "asc" ? "↑" : "↓"}
              </button>
              ...
              {(startDate || endDate) && (
                <button
                  ...
                  className="p-2 text-[#4A5043] hover:text-red-500 transition-colors"
                >
                  Clear Dates ×
                </button>
              )}
            ...
            {filteredAndSortedTickets.map((ticket) => (
              ...
              >
                <div 
                  ...
                  className="bg-white rounded-[24px] overflow-hidden cursor-pointer transition-all duration-300 hover:shadow-[0_8px_30px_rgb(0,0,0,0.12)] border border-[#D3D9C9] h-full"
                >
                  {/* Header Section */}
                  <div className="relative">
                    ...
                    <div className="absolute inset-x-0 bottom-0 p-4">
                      <div className="flex items-end justify-between gap-4">
                        ...
                        <div className="flex-shrink-0 bg-white rounded-xl p-2 text-center min-w-[60px]">
                          <p className="text-xl font-bold text-[#2C3228] leading-none">
                            {new Date(ticket.date).getDate()}
                          </p>
                          <p className="text-xs font-medium text-[#4A5043] uppercase mt-1">
                  ...
                  {/* Status Bar - With show time on right */}
                  <div className="bg-[#F4F6F0] px-4 py-3 flex items-center justify-between">
                    <div className="flex items-center gap-1.5">
                        <span className="w-1.5 h-1.5 bg-green-500 rounded-full animate-pulse"></span>
                        <span className="text-sm font-medium text-[#2C3228]">Live Sales</span>
                    </div>
                    <div className="flex items-center gap-3">
                        {userSubscription !== "free" && getDiscountBadge()}
                        <div className="flex items-center gap-1.5 bg-[#2C3228] text-white px-3 py-1.5 rounded-full">
                            <span className="text-xs">🎭 Show Time</span>
                            <span className="text-sm font-semibold">{ticket.time}</span>
                        </div>
                    </div>
                  </div>

                  {/* Show Details Button */}
                  <div className="px-4 py-2 border-b border-[#D3D9C9]">
                    <button 
                        onClick={(e) => {
                            e.stopPropagation();
                            handleTicketClick(ticket._id.toString());
                        }}
                        className="w-full flex items-center justify-between text-sm text-[#4A5043] hover:text-[#2C3228] transition-colors"
                    >
                        <span className="flex items-center gap-2">
                            <span>🎫</span>
                            <span>Show Event Details</span>
                        </span>
                        <span className="text-xs bg-[#F4F6F0] px-2 py-1 rounded-full">Click for more info</span>
                    </button>
                  </div>
                  ...
                  <div className="p-3 grid gap-2">
                    {ticket.seatCategories.map((category) => (
                        <button
                            ...
                            className={`w-full h-[72px] p-2 sm:p-3 rounded-xl text-left transition-all overflow-hidden ${
                                category.availableSeats > 0 
                                    ? "hover:bg-[#F4F6F0] border border-[#D3D9C9]" 
                            ...
                            <div className="flex items-center gap-2 sm:gap-4 h-full">
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-1 sm:gap-2 mb-0.5 sm:mb-1">
                                        <span className="font-medium text-[#2C3228] text-sm sm:text-base tracking-tight truncate">
                                            {category.name}
                                        </span>
                                        {category.availableSeats <= 20 && (
                                            <span className="inline-flex items-center gap-1 px-1.5 sm:px-2 py-0.5 text-[10px] sm:text-xs font-medium bg-red-100 text-red-600 rounded-full animate-pulse whitespace-nowrap">
                                                <span className="w-1 h-1 bg-red-600 rounded-full hidden sm:inline-block"></span>
                                                <span className="truncate">{category.availableSeats} left!</span>
                                            </span>
                                        )}
                                    </div>
                                    <div className="flex items-baseline gap-1 sm:gap-2">
                                        {userSubscription !== "free" && (
                                            <span className="text-xs sm:text-sm text-gray-400 line-through font-normal truncate">
                                                Rp {category.price.toLocaleString("id-ID")}
                                            </span>
                                        )}
                                        <div className="flex items-baseline gap-0.5 sm:gap-1 min-w-0">
                                            <span className="text-[10px] sm:text-xs font-normal text-[#4A5043]">Rp</span>
                                            <span className="text-base sm:text-lg font-semibold text-[#2C3228] tracking-tight truncate">
                                                {Math.round(calculateDiscountedPrice(category.price)).toLocaleString("id-ID")}
                                            </span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="flex-shrink-0 flex flex-col items-end gap-1">
                                    <div className={`
                                        h-[45px] sm:h-[52px] w-[45px] sm:w-[52px] rounded-xl flex flex-col items-center justify-center
                                        ${category.availableSeats <= 20 
                                            ? 'bg-red-50 text-red-600 border border-red-200 shadow-sm' 
                                            : 'bg-[#F4F6F0] text-[#2C3228] border border-[#D3D9C9] shadow-sm'}
                                    `}>
                                        <span className={`text-lg sm:text-xl font-semibold ${
                                            category.availableSeats <= 20 ? 'text-red-600' : 'text-[#2C3228]'
                                        }`}>
                                            {category.availableSeats}
                                        </span>
                                        <span className={`text-[8px] sm:text-[10px] font-medium ${
                                            category.availableSeats <= 20 ? 'text-red-500' : 'text-[#4A5043]'
                                        }`}>
                                            Seats
                                        </span>
                                    </div>
          ...
          <div className="space-y-4">
            {filteredAndSortedTickets.map((ticket) => (
              <div 
                key={ticket._id.toString()}
                onClick={() => handleTicketClick(ticket._id.toString())}
                className="bg-white rounded-2xl overflow-hidden border border-[#D3D9C9] hover:shadow-[0_8px_30px_rgb(0,0,0,0.12)] transition-all cursor-pointer"
              >
                <div className="flex">
                    {/* Left: Image Section */}
                    <div className="relative w-[280px] flex-shrink-0">
                        ...
                        <div className="absolute bottom-4 left-4 bg-white rounded-xl p-2 text-center min-w-[60px]">
                            <p className="text-xl font-semibold text-[#2C3228] leading-none">
                                {new Date(ticket.date).getDate()}
                            </p>
                            <p className="text-xs font-medium text-[#4A5043] mt-1">
                                {new Date(ticket.date).toLocaleDateString("id-ID", { month: "short" })}
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="flex-1 p-6">
                        ...
                        <div className="mb-4">
                            ...
                            <h3 className="text-2xl font-semibold text-[#2C3228] mb-1">{ticket.name}</h3>
                            <p className="text-[#4A5043] flex items-center gap-2">
                                <span>📍</span>
                                {ticket.venue}
                            </p>
                        </div>
                                    ...
                                    <div className="flex items-center justify-between gap-3">
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center gap-2 mb-1">
                                                <span className="font-medium text-[#2C3228] text-sm truncate">
                                                    {category.name}
                                                </span>
                                                {category.availableSeats <= 20 && (
                                                    <span className="px-1.5 py-0.5 text-[10px] font-medium bg-red-100 text-red-600 rounded-full animate-pulse whitespace-nowrap">
                                            ...
                                            <div className="flex items-baseline gap-2">
                                                {userSubscription !== "free" && (
                                                    <span className="text-xs text-gray-400 line-through font-normal truncate">
                                                        Rp {category.price.toLocaleString("id-ID")}
                                                    </span>
                                                )}
                                                <div className="flex items-baseline gap-0.5 min-w-0">
                                                    <span className="text-[10px] font-normal text-[#4A5043]">Rp</span>
                                                    <span className="text-sm font-semibold text-[#2C3228] truncate">
                                                        {Math.round(calculateDiscountedPrice(category.price)).toLocaleString("id-ID")}
                                                    </span>
                                                </div>
                                            </div>
                                        ...
                                        <div className={`
                                            h-[40px] w-[40px] rounded-lg flex flex-col items-center justify-center
                                            ${category.availableSeats <= 20 
                                                ? 'bg-red-50 text-red-600 border border-red-200' 
                                                : 'bg-[#F4F6F0] text-[#2C3228] border border-[#D3D9C9]'}
                                        `}>
